			...
			<div class="category-content" v-if="currentItem && currentItem.children">
				<ul>
					<li class="item" v-for="item in currentItem.children" v-bind:key="item.id">
						...
						<div>
							<dl v-if="item_1.status == 1" v-for="item_1 in item.children" :key="item_1.id" v-on:click="goProduct(item_1.id)"
								>
...
export default {
	...
	computed: {
		...mapState({
			items: state => state.keep.items,
			currentItem: state => state.keep.currentItem,
		}),
	...
	methods: {
		...mapMutations({
			keepSaveCurrentGoodsclassItem: 'keepSaveCurrentGoodsclassItem',
		}),
		...mapActions({
			keepGoodsclassList: 'keepGoodsclassList'
		}),
		loading() {
			if (!(this.items && this.items.length)) {
				Indicator.open();
			}
			this.keepGoodsclassList().then(
				response => {
					Indicator.close();
				},
				error => {
		...
		onItemClick(item) {
			this.keepSaveCurrentGoodsclassItem(item);
		},
		goProduct(id) {
			let params = { cate_id: id };
			this.$router.push({ name: 'KeepGoodslist', query: params });
		}
	}
};
</script>
