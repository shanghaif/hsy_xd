...
export default {
	...
	data() {
		return {
			liveData:[],  // 视频地址
			store: {
				store_id: this.$route.query.sid ? this.$route.query.sid : '12'
			},
			sxturl: '',
			sxt: {},
			player: '',
	...
	created() {
		// setTimeout(()=>{
		// 	this.liveData = this.sxt.cams;
		// 	console.log('全部',JSON.stringify(this.liveData));
		// },1000)
    var terminal_id = 3;
    if( isWechat() ){
      terminal_id = 2;
...
            }
						...
						// console.log('海康',JSON.stringify(that.liveData));
					},
					error: function(e) {
						console.log(e);
					}
			...
			that.liveData = that.sxt.cams;
			// console.log('大华',JSON.stringify(that.liveData));
		},
	}
};
</script>
