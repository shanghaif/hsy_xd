<template>
	<div class="MessageForm container hsy_main">
		<head-title title="信息处理"></head-title>
		<div>
			<!--投诉信息-->
			<div>
				<!-- <mt-cell class="menu-item" title="投诉信息"></mt-cell> -->
				<div class="menu-content">
					<mt-cell title="上报人" :value="complaint.safety_name"></mt-cell>
					<mt-cell title="上报标题" :value="complaint.text_title"></mt-cell>
					...
					<div @click="isshow = true" v-if="complain_pic.length"><mt-cell title="上报凭证" value="查看"></mt-cell></div>
					<br>
					
				<div v-if="!complaint.end_time"  class="body">
					<div class="order-comment-body">
						<div class="enter"><textarea placeholder="处理意见" v-model="desc"></textarea></div>
					</div>
				</div>
				<mt-cell v-if="complaint.end_time" title="处理时间" :value="new Date(complaint.end_time*1000).toLocaleString()"></mt-cell>
				<div  v-if="complaint.end_time" class="textacont">
					<div class="title">处理信息</div>
					<div class="enter">
						<textarea rows="14" readonly="true" placeholder="暂未处理" v-model="complaint.desc">
					</textarea></div>
				</div>
					<!-- <mt-cell title="处理信息" :value="complaint.complainsubject_desc?complaint.complainsubject_desc:'暂未处理'"></mt-cell> -->				
					
				<mt-button  v-if="!complaint.end_time" @click="fun_addReport" class="hsy_f hsy_btn">提交</mt-button>
				</div>
...
import { getReportInfo, saveReportMessage } from '../../../apikeep/report.js';
...
export default {
		...
		comText(){
			if( this.complaint ){
				if( this.complaint.text_desc ){
					return (parseInt(this.complaint.text_desc.length/14)<12)?parseInt(this.complaint.text_desc.length/14):12
				}
			}
		},
	},
		...
		if ( this.text_id ) {
			getReportInfo(this.text_id)
				.then(res => {
					...
					// this.complain_pic = ['http://shop.hsy.com/uploads/home/complain/788_2021012015042622268.jpg']//res.result.text_image;
				})
	...
	methods: {
		...
		fun_addReport() {
			saveReportMessage(this.text_id,this.desc)
				.then(res => {
					this.$router.back();
				})
				.catch(function(error) {
