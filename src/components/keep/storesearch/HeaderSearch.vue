...
<div class="brand_h_search">
	...
	<!-- <div class="header">
		<div class="header_cont">
			...
			<span class="header-title">{{title}}</span><span class="header-right"></span>
		</div>
	</div> -->
	<div class="header">
		<div class="header-wrap">
			<img class="img-back" @click="$router.go(-1)" src="@/assets/image/icon2/icon-back.png" alt="" />
...
export default {
	data() {
		return {
			...
			currenKeywords: this.utils.fetch(this.history),
			previous : null,
		};
	},
	props: ['keyword','title','place','history'],
	computed: {
		ifKeywords() {
			if (this.keywords === '') {
				return true;
	...
	methods: {
		...
		fun_disabled(){
			this.$emit(this.history,this.keywords);
			this.$router.go(-1);
		},
      ...
      if ( this.previous + 680 <= now ) {
				...
				if (value) {
					...
					this.utils.save(this.history, data);
				}
