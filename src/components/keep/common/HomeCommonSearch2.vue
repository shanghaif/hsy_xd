<template>
	<div class="product-list-header" id="bar" :class="headerFixed == true ? 'isFixed product-list-header' : ''">
		...
		<img class="icon-back" src="../../../assets/image/icon2/icon-back.png" @click="$router.go(-1)" />
...
export default {
	...
	methods: {
		...
		onSearch() {
			this.$router.push({ name: 'KeepSearch', params: { storeGoods: this.storeGoods } });
		},
		popupMore() {
...
.common-search input {
	...
	background: #eee url(../../../assets/image/icon2/search.png) no-repeat 2.17rem center;
