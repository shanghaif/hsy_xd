			...
			<div v-if="complaintList && complaintList.length">
				<div class="goods-item" v-for="(item, index) in complaintList" :key="index">
					...
					<div class="goods-info" :class="{goodsInfoEndTime:item.end_time}"
						@click="$router.push({ name: 'KeepReportForm', query: { complain_id: item.complain_id } })">
...
import { getComplaintListJuBao } from '../../../apikeep/report';
export default {
	name: 'ReportList',
	components: {
		EmptyRecord2,
	...
	methods: {
		getComplaintList(ispush) {
			...
			getComplaintListJuBao(params).then(res => {
				Indicator.close();
				if (res) {
