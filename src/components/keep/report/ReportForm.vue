...
import { getReportInfoJubao, saveReportMessageJubao } from '../../../apikeep/report.js';
import CropImg from '../../CropImg'

export default {
	data() {
		return {
			complainsubject_desc: '',   // 处理意见
			store_id :this.$route.query.store_id,    // 商家 id
			store_name :this.$route.query.store_name, // 商家 name

			complain_id : this.$route.query.complain_id,  // 举报 id
	...
	created() {
		if ( this.complain_id ) {
			getReportInfoJubao(this.complain_id)
				.then(res => {
					this.complaint = res.result;
					this.complain_pic = res.result.complain_images;
				})
	...
	methods: {
		...
		fun_addReport() {
			saveReportMessageJubao(this.complain_id,this.complainsubject_desc)
				.then(res => {
					this.$router.back();
				})
				.catch(function(error) {
