<template>
	<div class="container">
		...
		<div class="header" :style="{ backgroundImage: 'url(' + store.mb_title_img + ')' }">
			<div class="content-wrapper">
				<div class="avatar" @click="goStoreabout"><img :src="store.store_avatar" /></div>
				<div class="content">
					<div class="store_name">{{ store.store_name }}</div>
					<div v-if="(store.is_platform_store && config.business_licence) || store.business_licence_number_electronic" @click="imageVisible = true">
						<i class="iconfont">&#xe621;</i>
					</div>
				</div>
				...
				<i class="header-more iconfont" @click="popupMore">
					&#xe680;
		...
		<header-more v-show="popupVisible"
			@change-popupVisible="changePopupVisible" :showDot="showDot"></header-more>
		<mt-swipe class="mt-5" v-if="store.mb_sliders && store.mb_sliders.length" v-bind:style="getBannerStyle" :showIndicators="isShowIndicators">
			<mt-swipe-item v-for="(item, index) in store.mb_sliders" :key="index"><img :style="getBannerStyle" v-lazy="item.imgUrl" @click="goAd(item)" /></mt-swipe-item>
		</mt-swipe>
		<store-product-list :items="rec_goods_list" v-if="rec_goods_list && rec_goods_list.length > 0"></store-product-list>
    ...
    <header-more v-show="popupVisible"
      ...
      :showDot="showDot" :store='store' ></header-more>
    <mt-swipe class="mt-5" v-if="store.mb_sliders && store.mb_sliders.length" v-bind:style="getBannerStyle" :showIndicators="isShowIndicators">
    ...
    <mt-popup v-model="imageVisible" popup-transition="popup-fade" class="middle-popup">
		...
		<!-- 选择区域 -->
		<region-picker ref="picker" :closeModal="false" :valm="Area_id" v-on:onConfirm="onPickerConfirm" @onConfirmend="onConfirmend"></region-picker>

		<div v-if="c_show" class="ui_form_area">
			<span>请选择社区团</span>
			<div class="ui_form_control" @change="csel">
				<select class="ui_form_input" v-model="tid">
...
import HeaderMore from '../../HeaderMore2';
...
export default {
  ...
  methods: {
      ...
      if( this.areaId == null){//地址为空
        if(this.mclass == 0 ){ //未登录 
          ...
          cartType(this.memberInfo.member_id).then(cart=>{
            ...
            if( cart.result == null || cart.result.cart_type == null){//购物车无商品
              MessageBox({
                closeOnClickModal:false,//点击灰色禁止关闭
                title: '温馨提示',
                message: '普通会员查看商品类型',
            ...
            }else if( cart.result.cart_type == "l" ){//购物车为零售商品
              this.saveHomecartMarket(cart.result.cart_type);
              this.varMarket = cart.result.cart_type;
              this.onRegion();
            }else{//购物车为团购商品
              this.saveHomecartMarket(cart.result.cart_type);
              this.varMarket = cart.result.cart_type;
        ...
        if(this.mclass == 0 ){ //未登录 
          ...
          cartType(this.memberInfo.member_id).then(cart=>{//查询购物车商品类型
            ...
            if( cart.result==null || cart.result.cart_type == null){//无商品
              MessageBox({
                closeOnClickModal:false,//点击灰色禁止关闭
                title: '温馨提示',
                message: '普通会员查看商品类型',
    ...
    csel(){
      ...
      for(var i =0;i<this.c.length;i++){
        if(this.c[i].member_id == this.tid){
          if( this.mclass == 0){//未登录
            this.memberEx([this.exid,this.c[i].tuan])//暂时保存
          }else if(this.mclass <3){
            this.memberSaveTuan(this.c[i].tuan);//普通用户登录 永久保存
          }
          // return;
        }
      }
    ...
    loading(){
      ...
      var seller_id = null,member_id = null,areaId = this.areaId,tuan = this.tuan;
