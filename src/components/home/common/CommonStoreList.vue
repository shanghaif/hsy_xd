		...
		<div class="storelist-body show-store-list">
			...
			<div class="flex-wrapper" v-infinite-scroll="getMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
				<div class="ui-product-body" v-for="(item, index) in storeList" v-bind:key="index">
					<div class="list-wrapper" v-on:click="goDetail(item)">
						<div class="list">
									...
									<div class="store-info">
										...
										<div class="sub-title" style="-webkit-box-orient:vertical">
											<span class="common-score-wrapper">
												...
												<!-- <i class="iconfont front" :style="{ width: item.store_credit_percent + '%' }"></i>
												<i class="iconfont back"></i> -->
											</span>
										</div>
									</div>
									<!-- <div class="btn" :class="{ active: item.is_favorate }">
										<i class="iconfont">&#xe610;</i>
										 关注 
									</div> -->
								</div>
							...
							<div class="btn" @click.stop="toggleFavorite(item)">
								<img :src="item.is_favorate ? heart1 : heart2" alt="" />
								<span :class="{ active: item.is_favorate }">{{ item.is_favorate ? '已关注' : '关注' }}</span>
								<img v-show="item.is_camer == '1'" class="i-video" :src="require('@/assets/image/icon2/store-camm.png')" alt="">
							</div>
						</div>
...
export default {
	...
	methods: {
				...
				if (window.plus) {
					...
					plus.geolocation.getCurrentPosition(
						p => {
							...
							// Toast('获取成功：'+ that.member_point.lat+ '****'+that.member_point.lng);
		...
		getStoreList(ispush = false, nearby = false) {
			...
			if (nearby) {
				getStoreNearbyList(data.brand, data.category, data.keyword, this.member_point.lng, this.member_point.lat, data.sort_key, data.page, this.token,this.store_area_id).then(res => {
					this.buildData(ispush, res);
				});
			} else {
				getStoreList(data.brand, data.category, data.keyword, 0, 0, data.sort_key, data.page, this.token, this.store_area_id).then(res => {
					this.buildData(ispush, res);
				});
			}
		},
		...
		toggleFavorite(item) {
			if (!item.is_favorate) {
				addFavoriteStore(item.store_id).then(
					response => {
						Toast(response.message);
						item.is_favorate = !item.is_favorate;
						item.store_collect += 1;
					},
					error => {
						Toast(error.message);
					}
				...
				delFavoriteStore(item.store_id).then(
					response => {
						Toast(response.message);
						item.is_favorate = !item.is_favorate;
						item.store_collect -= 1;
