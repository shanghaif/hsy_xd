	...
	<div class="container">
		...
		<div class="header">
			<div class="header-wrap">
				<img class="img-back" @click="$router.push({ name: 'HomeIndex' })" src="@/assets/image/icon2/icon-back.png" alt="" />
				<span class="header-title">商家列表</span>
				<!-- <span></span> -->
				<img class="img-search" @click="$router.push( {name:'HomeStoresearch'} )" src="@/assets/image/icon2/search.png" alt="" />
			</div>
		</div>
		...
		<div class="ui-category-body">
			<div class="category-flex">
				<div ref="leftCont" class="category-sidebar" :class="isClose?'closeLeft':''">
					<ul class="cont">
						...
						<!-- <li class="item_title">
							...
							<li
								class="item"
								v-for="item in storeclass_list_cy"
								v-bind:key="item.storeclass_id"
								v-on:click="openclass(item.storeclass_id)"
								v-bind:class="{ sidbaractive: item.storeclass_id == currentid, noActive: item.storeclass_id != currentid }"
							>
						...
						<li class="item_title">批发</li>
							<li
								class="item"
								v-for="item in storeclass_list_pf"
								v-bind:key="item.storeclass_id"
								v-on:click="openclass(item.storeclass_id)"
								v-bind:class="{ sidbaractive: item.storeclass_id == currentid, noActive: item.storeclass_id != currentid }"
							>
						...
						<li class="item_title">零售</li>
							<li
								class="item"
								v-for="item in storeclass_list_ls"
								v-bind:key="item.storeclass_id"
								v-on:click="openclass(item.storeclass_id)"
								v-bind:class="{ sidbaractive: item.storeclass_id == currentid, noActive: item.storeclass_id != currentid }"
							>
						...
						<li class="item_title">生产</li>
							<li
								class="item"
								v-for="item in storeclass_list_sc"
								v-bind:key="item.storeclass_id"
								v-on:click="openclass(item.storeclass_id)"
								v-bind:class="{ sidbaractive: item.storeclass_id == currentid, noActive: item.storeclass_id != currentid }"
							>
						...
						<li class="item_title">药店</li>
							<li
								class="item"
								v-for="item in storeclass_list_yd"
								v-bind:key="item.storeclass_id"
								v-on:click="openclass(item.storeclass_id)"
								v-bind:class="{ sidbaractive: item.storeclass_id == currentid, noActive: item.storeclass_id != currentid }"
							>
						...
						<li class="item_title">农产农资</li>
							<li
								class="item"
								v-for="item in storeclass_list_nz"
								v-bind:key="item.storeclass_id"
								v-on:click="openclass(item.storeclass_id)"
								v-bind:class="{ sidbaractive: item.storeclass_id == currentid, noActive: item.storeclass_id != currentid }"
							>
				...
				<div ref="rightCont" class="category-content">
					<common-store-list @haveStoreData="haveStoreData" ref="child" :currentid="currentid"></common-store-list></div>
			</div>
		</div>
	</div>
</template>
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    // 在导航完成前获取数据
    next(vm => {
			( from.name != 'HomeStoreFoodslist' && from.name != 'HomeStoreGoodslist' )&&(
	...
	methods: {
		// 接收 传值
		haveStoreData(){
			// console.log('点我了',this.leftContScrollTop );	
			// console.log('点我了',this.righContScrollTop );			
