...
export default {
  data () {
    return {
      ...
      upgrape: this.$route.query.upgrape ? this.$route.query.upgrape : '',
      hideFooter: false,
      popupVisible: true,
      currentScore: 0,
  ...
  created () {
		this.saveUpGrape(this.upgrape);
    // form_id, 入口, 0无, 1折扣, 2抢购
    // terminal_id, 终端, 1APP, 2公众号, 3 h5
    var form_id = 0;
    var terminal_id = 3;
    ...
    this.getGoodsDetail({terminal_id:terminal_id,form_id:form_id, goods_id: this.productId, token: this.token, extra: extra }).then(res=>{
      this.ifLoad=true
    }).catch(error => {
      Toast(error.message)
    })
