<template>
<div class="scan">  
  <img class="back" @click="$router.go(-1)" src="@/assets/image/icon2/icon-black-left.png" alt="" />
  <div :style="{height:select.h0}">    
...
export default {
  ...
  methods: {
    ...
    startRecognize () {
      ...
      var styles = {frameColor: "#FFFFF1",scanbarColor: "#FFFFF1",background: ""};
      ...
      function onmarked (type, result, file) {
        ...
        that.closeScan()
        scan.close()
        // https://h.ihaosy.com/home/traceability?goods_commonid=954
        // 如果 是扫描的 商品追溯信息
        if(result.indexOf('traceability?goods_commonid=') !=-1 ){
          var commonid = result.split('traceability?goods_commonid=')[1];
          that.$router.push({ name: 'HomeTraceability' ,query:{goods_commonid:commonid} })
        }else{
