...
<template>
	...
	<div class="footer" v-else>
		...
		<div class="ok-submit" @click="$router.go(-1)">完成</div>
	</div>
</template>
...
export default {
	...
	computed: {
		totalPrice() {
			return this.banquetStore[this.table_name+'Total'];
		},
		...mapState({
		...
		allPrice() {
			console.log(this.cartNumber*this.price,this.cartNumber,'this.cartNumber*this.price')
			return this.cartNumber*this.price
		},
		getinformationCount() {
	...
	methods: {
		...
		checkout() {
			...
			let m_per = 1;
			if (this.mclass == 1) {
				m_per = this.detailInfo.goods_l_price;
			}
			if (this.mclass == 2) {
				m_per = this.detailInfo.goods_c_price;
			}
			if (this.mclass == 3 || this.mclass == 6) {
				m_per = this.detailInfo.goods_p_price;
			}
			if (this.mclass == 4) {
				m_per = this.detailInfo.goods_B_price;
			}
			if (this.mclass == 5) {
				m_per = this.detailInfo.goods_j_price;
			}
