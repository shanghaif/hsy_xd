		...
		<div class="ui-detail-info">
			...
			<div class="spike" v-if="mclass < 3 && detailInfo.promotion_type == 'groupbuy'">
				...
				<div class="spike_left">
					<div class="img_bg"><img src="@/assets/image/icon2/spike_tit.png" alt="" /></div>
			...
			<div class="info-header ui-flex" v-else>
				...
				<div class="food-add" v-if="isFrom == 'dinner'">
					<span class="subtract" @click="deleteNum">-</span>
					<span class="num">{{ obj.num }}</span>
					<span class="add" @click="addNum($event)">+</span>
...
export default {
	data() {
		return {
			...
			priseName: '',
			pricePer: 100,
			mclassGoodsPrice:"good_price",
			realprice: 0,
			isFrom: this.$route.query.isFrom ? this.$route.query.isFrom : '',
			selectNum: 1,
			obj: {},
			cartData: '',
	...
	computed: {
		...mapState({
			...
			banquetObj: state => state.home.banquetObj,
		}),
		favoriteName() {
	...
	watch: {
		selectNum(val) {
			console.log(val);
			// this.saveDinnerFoodNum(val)
		},
	},
	created() {
		if( this.$route.query.isFrom == 'dinner'){
			this.obj = this.banquetObj;
			if (this.mclass == 1) {
				this.mclassGoodsPrice = 'goods_l_price';
			}
			if (this.mclass == 2) {
				this.mclassGoodsPrice = 'goods_c_price';
			}
			if (this.mclass == 3 || this.mclass == 6) {
				this.mclassGoodsPrice = 'goods_p_price';
			}
			if (this.mclass == 4) {
				this.mclassGoodsPrice = 'goods_p_price';
			}
			if (this.mclass == 5) {
				this.mclassGoodsPrice = 'goods_j_price';
			}
			// 菜品
			// 购物车保存的 菜品
			this.cartData = this.banquetStore[this.table_name];
			for (var iend of this.cartData ) {
				if ( iend.goods_id == this.goods_id) {
					this.obj = iend;
					this.$forceUpdate();
				}
			}
	...
	methods: {
		...mapMutations({
			...
			// saveDinnerFoodNum:'saveDinnerFoodNum'
		}),
		subtract() {
			// this.$parent.reduce;
			// this.selectNum--;
		...
		saveToStore(){
			...
			// if( this.table_name == 'cook' ){
				for( var item of this.cartData ){
					cookTotal += item.num*( parseFloat(item.goods_price*item[this.mclassGoodsPrice]).toFixed(2));
				}
			// }else{
				// console.log(this.banquetStore);
				for( var item of this.banquetStore['xz'] ){
					xzTotal += item.num*( parseFloat(item.goods_price*item[this.mclassGoodsPrice]).toFixed(2));
				}
				for( var item of this.banquetStore['dz'] ){
					dzTotal += item.num*( parseFloat(item.goods_price*item[this.mclassGoodsPrice]).toFixed(2));
				}
				for( var item of this.banquetStore['bj'] ){
					bjTotal += item.num*( parseFloat(item.goods_price*item[this.mclassGoodsPrice]).toFixed(2));
				}
			// }
			var obj = {
				key:this.table_name,
		...
		addNum() {
			...
			if (this.cartData.length > 0) {
				for (var obj of this.cartData) {
					// 如果当前 id  在 购物车里
					if (obj.goods_id == this.obj.goods_id) {
						hasValue = true;
					}
				}
				if (hasValue) {
