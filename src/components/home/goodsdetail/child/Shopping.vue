		...
		watchNumberPrice(){
			if (this.detailInfo.promotion_type == 'groupbuy' && ((+this.detailInfo.upper_limit)==0 || (+this.number) <= (+this.detailInfo.upper_limit)) && this.mclass<3) {
				...
				if(this.detailInfo.promotion_type == 'xianshi' && (+this.number) >= (+this.detailInfo.lower_limit) && this.mclass<3){
					...
					var moneryType;
					if (this.mclass == 1) {
						moneryType = this.detailInfo.goods_l_price;
					}
					if (this.mclass == 2) {
						moneryType = this.detailInfo.goods_c_price;
					}
					if (this.mclass == 3 || this.mclass == 6) {
						moneryType = this.detailInfo.goods_p_price;
					}
					if (this.mclass == 4) {
						moneryType = this.detailInfo.goods_p_price;
					}
					if (this.mclass == 5) {
						moneryType = this.detailInfo.goods_j_price;
					}
