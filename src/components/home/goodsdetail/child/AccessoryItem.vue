<!-- with.vue -->
<template>
	<div class="ui-recommend-wrapper" v-if="bundling_array && mclass <3">
		<p class="title">
			<span></span>
			<span>优惠套装</span>
		</p>
		<div class="wrapper-swipe">
			<mt-swipe v-bind:style="getWrapperStyle" :auto="0" @change="handleChange" :default-index="currentIndex" :stop-propagation="true" :speed="100" :showIndicators="false">
				<mt-swipe-item v-for="(item, index) in bundling_array" :key="index">
					<div class="image-swipe-wrapper">
						<div v-for="(image, j) in b_goods_array[index]" @click="goDetail(image.id)" v-bind:style="getItemStyle">
							<img :src="image.image" />
...
export default {
	...
	methods: {
		...
		getRecommendList() {
			...
			productAccessoryList(params.product,this.token).then(res => {
				if (res) {
