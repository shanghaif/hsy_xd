...
<template>
	...
	>
		<div class="footer-flex">
			...
			<div class="right" v-if="mclass>0 && CanAddCart">
				<div
					class="button active-cart"
					v-on:click="addShopping(true)"
					v-if=" price!=0 && !detailInfo.bargain_info && ((!detailInfo.pintuan_type && detailInfo.goods_storage > 0 && detailInfo.cart) || (detailInfo.pintuan_type && detailInfo.pintuangroup_list.length))"
				>
					{{detailInfo.pintuan_type?'参加拼团':'加入购物车'}}
				</div>
				<div class="button disabled-cart"
             v-on:click="addShopping(true)"
             v-if=" price!=0 && !detailInfo.bargain_info && ((!detailInfo.pintuan_type && detailInfo.goods_storage <= 0 && detailInfo.cart) || (detailInfo.pintuan_type && detailInfo.goods_storage <= 0 && detailInfo.pintuangroup_list.length))">
					{{detailInfo.pintuan_type?'参加拼团':'加入购物车'}}
				</div>
				<div
					class="button active-buy"
					v-on:click="checkout()"
					v-if=" price!=0 && detailInfo.goods_storage > 0"
				>
...
export default {
  ...
  methods: {
    ...mapMutations({
      ...
      saveSelectedCartGoods: 'saveSelectedCartGoods'
		...
		xgprise(){
			this.price=10	
			if(this.mclass==1){
				this.price = this.detailInfo.goods_l_price
			}
			if(this.mclass==2){
				this.price = this.detailInfo.goods_c_price
			}
			if(this.mclass==3){
				this.price = this.detailInfo.goods_p_price
			}
			if(this.mclass==4){
				this.price = this.detailInfo.goods_B_price
			}
			if(this.mclass==5){
				this.price = this.detailInfo.goods_j_price
			}
    ...
    // 立即购买
    checkout () {
			if(this.mclass <3 && !this.area_id){	//如果 没有默认收货地址
				Toast('请添加默认收货地址');
				setTimeout(()=>{			
