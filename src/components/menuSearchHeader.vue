<template>
	<div class="product-list-header">
		...
		<div class="common-search"><input type="text" placeholder="请输入您要搜索的菜谱" autocomplete="off" v-model="keywords" /></div>
...
export default {
	data() {
		return {
			...
			currenKeywords: this.utils.fetch('menukeyword'),
			// showMunuList: this.$route.params.menu ? this.$route.params.menu : ''
		};
	},
	props: ['keyword'],
	computed: {
	...
	methods: {
		...
		search(value) {
			...
			if (value) {
				...
				this.utils.save('menukeyword', data);
			}
			/* if(this.$route.path == '/home/menusearch') {
				// this.$emit('onChange',this.keywords)
			} else { */
				this.$router.push({
					name: 'MenuSearchList',query: { keywords: this.keywords }
				});
			// }
			// debugger
		},
