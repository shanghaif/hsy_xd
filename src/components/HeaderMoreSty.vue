<template>
	<div class="more-box sty">
		...
		<div class="more-item" @click="report">
			<!-- <i class="iconfont">&#xe625;</i> -->
			<img src="@/assets/image/icon2/icon-share.png" alt="">
			<span>举报</span>
		</div>
...
export default {
	...
	methods: {
		// 举报商家
		report(){
			// console.log('年后',this.store);
			if( this.isOnline ){
				this.$router.push( { name:'MemberReportForm' , query:{store_id:this.store.store_id,store_name:this.store.store_name}})
			}else{
				Toast("请先去登录");
			}
		},
