...
<div>
  <div class="step2" :class="{popMap_mmap:popMap}"  v-show="cropDataShow">
    ...
    <mt-field class="import" label="详细地址" v-model="storeJoinin.company_address_detail"></mt-field>
	  <mt-field class="import"  label="注册资金" v-model="storeJoinin.company_registered_capital"></mt-field>
...
import { getStoreJoinin, uploadInfoFile, saveStoreJoinin2, getStcomByxkz, uploadCropImg } from '../../../api/seller'
...
export default {
  ...
  methods: {
    ...
    nextStep () {
      ...
      // if( !this.storeJoinin.b_fzrjkz ){
      //   Toast('请上传负责人健康证');
      //   return false;
      // }
      // if( !this.storeJoinin.b_aqyjkz ){
      //   Toast('请上传安全员健康证');
      //   return false;
      // }
      // // 如果 已经查证过许可证编号 并且 给予了提示信息
      // if( this.b_xk_noType ){    
      ...
      //   // 否则 第一次 点击 提交
      //   // 如果 是 2020/9/1 之后的
      //   if( this.b_xk_noTime > 1598889600000 ){
      //     saveStoreJoinin2(this.storeJoinin).then(res => {
      //       this.sellerJoininSave({ storeJoinin: this.storeJoinin })
      //       this.$router.push({ name: 'SellerJoinin3' })
      //     }).catch(error => {
