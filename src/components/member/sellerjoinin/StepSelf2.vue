...
<div>
  ...
  <div class="step2" v-show="cropDataShow">
    ...
    <div class="label">店铺及法人信息</div>
    <mt-field class="import" :label="storeJoinin.store_type==1?'店铺名称':'入驻名称'" v-model="storeJoinin.company_name"></mt-field>
	...
	<mt-field label="注册资金" v-model="storeJoinin.company_registered_capital"></mt-field>
    ...
    <div class="label">经营信息（副本）</div>
    <mt-field label="许可证编号" v-model="storeJoinin.b_xk_no"></mt-field>
    <mt-field class="import" label="身份证号" v-model="storeJoinin.business_licence_number"></mt-field>
    <!-- <div class="import" @click="onRegion('business_licence_address')"><mt-cell title="身份证所在地" is-link>{{storeJoinin.business_licence_address}}</mt-cell></div> -->
    <div class="import" @click="onDate('business_licence_start')"><mt-cell title="身份证起始日期" is-link>{{storeJoinin.business_licence_start}}</mt-cell></div>
    <div class="import" @click="onDate('business_licence_end')"><mt-cell title="身份证结束日期" is-link>{{storeJoinin.business_licence_end}}</mt-cell></div>
    <div class="import pink">
      <mt-cell title="身份证长期有效"><mt-switch v-model="idSwitch" style="margin-right:8px"></mt-switch></mt-cell>      
    </div>
    <!-- <mt-field class="import"  label="经营范围" v-model="storeJoinin.business_sphere"></mt-field> -->
    <mt-cell class="import pt-5" title="身份证正面照">
    ...
    <mt-cell class="import pt-5" title="身份证反面照">
      <div class="user-avatar-wrapper">
    ...
    <mt-cell class="pt-5 import" title="负责人健康证">
      <div class="user-avatar-wrapper">
    ...
    <mt-cell class="pt-5 import" title="安全员健康证">
      <div class="user-avatar-wrapper">
...
import { getStoreJoinin, uploadInfoFile, saveStoreJoinin2, uploadCropImg } from '../../../api/seller'
...
export default {
  ...
  data () {
    return {
      ...
      options: [{ title: '入驻信息' }, { title: '财务资质' }, { title: '店铺信息' }, { title: '合同签订' }],
      showimage: '',
      isshow: false,
      popMap: false,
      selectData: '',
  ...
  methods: {
    ...
    // 是否符合身份证
    getIdCode(){
      return(this.storeJoinin.business_licence_number && /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i.test(this.storeJoinin.business_licence_number))
    },
    nextStep () {
      ...
      // if( this.storeJoinin.company_registered_capital != null && (!/^[0-9]*$/.test(this.storeJoinin.company_registered_capital)) ){
      //   Toast('注册资金必须为正整数');
      //   return false;
      // }
      ...
      saveStoreJoinin2(this.storeJoinin).then(res => {
        ...
        this.$router.push({ name: 'SellerJoininSelf3' })
      }).catch(error => {
        Toast(error.message)
      })
    },
