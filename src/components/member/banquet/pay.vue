<template>
	<div class="container">
		...
		<div v-else style="background-color: #fff;padding-top: 4.76rem;"><button class="ds-button-large" type="primary" v-on:click="$router.push({name:'MemberBanquetList', query: { state: 'state_new' } })">去待支付页面支付</button></div>
	</div>
</template>

<script>
import { resetRem, resumeRem } from '@/util/rem';
import { pay,getOrderpayInfo } from '../../../api/memberBanquet';
import { getVrOrderpayInfo } from '../../../api/memberVrBuy';
import { mapState } from 'vuex';
import {  getPaymentList, getPayData, wxpublicPayData } from '../../../api/memberPayment';
				...
				.catch(error => {
					...
					if( error.code == 10001 || error.message == "该订单不存在"){
						...
						if( _this.payment_info.payment_code == 'wxpay_h5' ){
							_this.$router.push({name:'MemberBanquetList', query: { state: 'state_new' ,payType:'wxapp'} })
						}else{
							_this.$router.push({name:'MemberBanquetList', query: { state: 'state_new' } })
							// this.$router.push({name:'MemberBanquetPay', query: { pay_sn: '订单号', pay_type:"pay_new" } })
					...
					if (error.code === 12001) {
						...
						_this.$router.push({ name: 'MemberBanquetList' });
					}
				});
		},
		//////////////////////////////////////////////////////////////////////////////
