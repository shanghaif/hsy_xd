...
<div class="chef-order">
		<!-- 顶部返回栏 -->
		<head-title fontSize='0.32rem' title="厨师订单"></head-title>
    <!-- 选择框 -->
      ...
      <div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
        <ul v-if="order_list && order_list.length">
          <li class="item" @click="$router.push({ name: 'MemberChefOrderDetail',query:{order_id:item.order_id} })"
            ...
            <div class="info">
              <div class="time">
                <span>宴请日期: 
                  <span :class="{redClass:new Date(item.banquet_time*1000).toLocaleDateString() == new Date().toLocaleDateString()}">
                    {{new Date(item.banquet_time*1000).toLocaleString()}}</span></span>
                <span>宴请方式: {{item.banquet_mode==1?"到店堂食":"厨师上门"}}</span>
              </div>
              <div class="time">
                <span>桌数：{{item.table_num}}</span>
                <span>菜品食材：{{item.dishes_food==1?"厨师准备":"家里准备"}}</span>
              </div>
    ...
    <!-- 否则 没有数据 -->
    <div class="empty-order" v-if="allShow" v-show="order_list&&!order_list.length">
...
import { getMemberBanqueCookList } from '../../../api/memberBanquet';
import { getOrderList } from '../../../api/memberOrder';
    ...
    close(item){
      ...
      item.type = false;
		...
		getOrderList(ifReplace) {
			...
			getMemberBanqueCookList(this.params)
				.then(res => {
          Indicator.close();
          this.allShow = true;
...
.chef-order{
  ...
  &/deep/.head_title_cont{
    height: 0.90rem;
    .header-wrap{
      padding: 0.26rem 0.30rem 0.24rem;
      img{
        width: 0.40rem;
        height: 0.40rem;
      }
