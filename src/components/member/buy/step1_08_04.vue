...
<div class="container">
    ...
    <div class="body" v-if="store_final_total_list">
        ...
        <div class="sendtime">        
          ...
          <mt-cell title="配送时间" ></mt-cell>
        </div>
        <!-- <div  @click="onDate4" class="sendtime">        
          <i class="icon iconfont">&#xe633;</i>
          <mt-cell title="配送结束时间" is-link>{{sendTime?$moment(sendTime).format('YYYY-MM-DD HH:mm:ss'):''}}</mt-cell>
        </div> -->
        <div class="sendtime">
          <a class="mint-cell">
            <!-- <span class="mint-cell-mask"></span>
            <div class="mint-cell-left"></div> -->
            <div class="mint-cell-wrapper" style="display: flex;justify-content: space-between;flex-wrap: wrap;padding: 1px 14px;">
              <div @click="onDate1" class="mint-cell-value endtime">{{sendTime?$moment(sendTime).format('YYYY-MM-DD HH')+"时":'配送起始时间'}}</div>
              <span>一</span>
              <div @click="onDate4" class="mint-cell-value endtime">{{endTime?$moment(endTime).format('YYYY-MM-DD HH')+"时":'配送结束时间'}}</div>
            </div>
...
export default {
  ...
  methods: {
    ...
    dateConfirm4(v){
      ...
      this.endTime = this.endTime - this.endTime%3600000;
      if( this.sendTime>this.endTime ){
        Toast("起始时间不得小于结束时间");
        this.endTime = this.sendTime + 3600000;
      }
    },
