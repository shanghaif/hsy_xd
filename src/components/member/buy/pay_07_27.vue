...
export default {
  ...
  mounted(){
    ...
    if (!window.plus) {     
      ...
      if (ua.match(/MicroMessenger/i) == "micromessenger") {//如果在微信里
        this.payment_list =  [
          {payment_code: "wxpublicpay", payment_name: "公众号支付"},
          {payment_code: "unionpay", payment_name: "支付宝支付"}
        ];
        this.WXloading();
      } else {// 在普通浏览器里
        this.payment_list =  [
      ...
      this.payment_list =  [
        {payment_code: "wxpay_h5", payment_name: "微信支付"},
        {payment_code: "unionpay", payment_name: "支付宝支付"}
      ];         
      this.payment_info.payment_code = this.payment_list[0].payment_code;
      this.getChannels();
  ...
  methods: {
    ...
    submit () {
      var pay_type_new;
      if( this.payment_info.payment_code == 'wxpay_h5' ){//如果是微信支付
        pay_type_new = "app_wx";
        // this.weixinPay()
        // return false;
