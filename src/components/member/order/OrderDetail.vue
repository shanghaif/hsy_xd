	...
	<div>
		...
		<div class="order-body" v-if="order_info">
			<div class="image">
				...
				<span class="endTime">{{ endTimeClose }}</span>
			</div>
			...
			<div class="content">
				...
				<div class="containers" v-for="item in order_info.goods_list" v-bind:key="item.goods_id">
					...
					<div class="right-wrapper">
						...
						<div class="desc-wrapper">
							...
							<div class="btn-list">
								<span v-if="order_info.if_complain" v-on:click="complaint(order_info.order_id, item.rec_id)">投诉</span>
								<span v-if="item.refund == '1' && order_info.order_state != 30" v-on:click="showRefund(item.rec_id)">售后</span>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="desc section-header section-footer">
					<div class="container">
						...
						<label class="subtitle"><span>￥</span><span>{{ order_info.goods_amount }}</span></label>
					</div>
					<div class="container">
						<label class="title">运费</label>
						<label class="subtitle"><span>￥</span><span>{{ order_info.shipping_fee }}</span></label>
					</div>
					...
					<label class="amount">
						实付款 :
						<span><span>￥</span><span>{{ order_info.order_amount }}</span></span>
					</label>
				</div>
			</div>
		</div>

		<mt-popup v-model="popupRefund" position="right" class="common-popup-wrapper shouhou">
			<div class="common-header-wrap">
				<mt-header title="售后" class="common-header"><mt-button slot="left" icon="back" @click="popupRefund = false"></mt-button></mt-header>
			</div>
...
import { getOrderInfo, cancelOrder } from '../../../api/memberOrder';
export default {
		...
		if (this.$route.query.order_id) {
			getOrderInfo(this.$route.query.order_id)
				.then(res => {
					...
					if (this.order_info.state_desc == '待付款') {
						//如果是待付款
						this.endTimeFun(); //调取订单是否超时函数
	...
	methods: {
		...
		endTimeFun() {
			...
			var oldTime = new Date(this.order_info.add_time).getTime(); //提交订单时间
			if (newTime - oldTime < 1800000) {
				//如果未超时
				var endTime = 1800000 - (newTime - oldTime);
				var m = Math.floor((endTime / (1000 * 60)) % 60); //计算分钟数
				var s = Math.floor((endTime / 1000) % 60); //计算秒数
				this.endTimeClose = '剩' + m + '分' + s + '秒自动关闭此订单';
			} else {
				//否则超超时了
				cancelOrder(this.order_info.order_id).then(res => {
					//取消订单
					this.$router.push({ name: 'MemberOrderList', query: { state: 'state_new' } }); //跳转到待支付界面
				});
			}
		},
		showRefund(rec_id) {
...
.image {
	// background: #e93b3d;
	background: url('../../../assets/image/icon2/icon-dingdan-bg.png') center center/100% 100% no-repeat;
	height: 17.67rem;
	display: flex;
