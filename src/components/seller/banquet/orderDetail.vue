	...
	<div class="container order-detail">
		...
		<div class="content">
			...
			<div class="store-info">
				...
				<div class="user-info" v-if="dinnerType == 1">
					<span style="margin-right: 1rem;width: 8rem;">上门地址:</span>
			...
			<div class="dishes" v-show="listInfo.goods_list&&listInfo.goods_list.length">
				<p>菜品</p>
			...
			<div class="list-cell-wrap" v-if="status == 3 || status == 2 || status == 4">
				...
				<div class="list-cell-start">
					<span>可退订时限:</span>
					<div v-if=" this.day > 0 || this.hours>0">{{ this.day > 0 ? this.day + '天' : this.hours + '小时' }}</div>
					<div v-else>请联系商家</div>
					<!-- 如果小于24小时，按小时显示哦 -->
				</div>
			</div>
		</div>
		<div class="footer" :style="{padding:status==1||status==2||status==3||status==4||statusing==5?'0.67rem 1.43rem':'0'}" v-if="status==1||status==2||status==3||status==4||statusing == 5">
			<div class="cancle botton" @click="cancle()" v-if="status == 1 ">取消订单</div>
			<div class=" botton deposit-money" v-if="status == 1 && statusing != 5" @click="receiving()">{{ dinnerType == 1 ? '选择厨师并接单' : '接单' }}</div>
			<div class="cancle botton" @click="writeMarket" v-if="status == 2 || status == 3 || status == 4 || statusing == 5">订单备注</div>
			<div class="botton " v-if="status == 2 && statusing != 5 && selectCookStatus == 1&&dinnerType==1" @click="(receivingStepValue = true), (selectCookStatus = 0)">修改厨师</div>
			<!-- <div class="botton deposit-money" style="margin-left: 1.43rem;"  v-if="dinnerType==1 && status == 3 && statusing != 5 && new Date(listInfo.banquet_time).toLocaleDateString() == new Date().toLocaleDateString()">签到</div> -->
			<!-- <div class="cancle"  v-if="">订单备注</div> -->
			<div class="botton" v-if="statusing == 5" @click="auditPopup = true">退订审核</div>
		</div>
		...
		<mt-popup class="receiving-value" v-model="receivingStepValue" position="bottom">
			...
			<!-- <div class="empty-cook">
				<img class="" src="@/assets/image/icon2/icon-empty-friend-list.png" alt="">
				<div>您还未添加厨师</div>
			</div> -->
			...
			<div v-if="cookList&&cookList.length > 0" class="confirm-get" @click="gostep2()">确定</div>
		...
		<mt-popup class="audit-popup" v-model="auditPopup" position="bottom">
			...
			<div class="result">
				<span>审核结果</span>
				<div>
					<span :class="selectOne ? 'select' : ''" @click="selectClick(2)">通过</span>
					<span :class="selectOne ? '' : 'select'" @click="selectClick(3)">驳回</span>
				</div>
			</div>
			<div class="list-cell"  v-if="this.index==2">
				<span>退订金额:</span>
				<div>
					<input type="text" placeholder="请输入给用户退回订金金额" v-model="tuiMoney" />元
				</div>
			</div>
			<div class="confirm-get" @click="tuiConfirm()">确定</div>
		</mt-popup>
	</div>
</template>

<script>
import { dinnerCookList, getListInfo, getOrderLInfo, getOrderCInfo,sendMarket } from '@/api/sellerBanquet';
...
export default {
	data() {
		return {
			...
			selectCookStatus: 1, //1选择厨师接单，2填写截单信息,0修改厨师
			cookList: [{ name: '11' }],
	...
	methods: {
		...
		tuiConfirm() {
			let params = {};
			params.order_id = this.order_id;
			params.conduct_status = this.index;
			if(this.index==2) {
				if(this.tuiMoney) {
					params.unsubscribe_money = this.tuiMoney
				}else {
					Toast('请输入可退订金额')
					return;
				}
		...
		timeDown() {
			if (this.listInfo.banquet_time) {
				...
				// const addTime = new Data(this.listInfo.add_time*100)
				...
				console.log(this.listInfo.time_limit * 24 * 60 * 60 * 1000,this.listInfo.time_limit)
				// let leftTime = parseInt((endTime.getTime() - this.listInfo.time_limit * 24 * 60 * 60 * 1000 - nowTime.getTime()) / 1000);
				...
				console.log(this.day,'aaa', this.hours);
