<template>
	<div class="member-information">
		<div class="common-header-wrap">
			<mt-header :title="'处理' + title" class="common-header"><mt-button slot="left" icon="back" @click="$router.go(-1)"></mt-button></mt-header>
		</div>

		<div class="form">
			<mt-cell :title="'同意' + title"><mt-radio v-model="seller_state" :options="seller_state_options"></mt-radio></mt-cell>
			<mt-cell v-if="refund_type == 2" title="弃货"><mt-radio v-model="return_type" :options="return_type_options"></mt-radio></mt-cell>
...
export default {
	...
	methods: {
		submitInformation() {
			editRefund(this.refund_id, this.return_type, this.seller_message, this.seller_state)
				.then(res => {
					Toast(res.message);
					this.$router.go(-1);
				})
