<template>
	<div class="member-order-list">
		...
		<head-title :title="title+'列表'"></head-title>
			...
			<div class="order-body" v-if="refund_list && refund_list.length">
				<div class="list" v-for="(item, index) in refund_list" v-bind:key="item.refund_id">
					<div @click="$router.push({ name: 'SellerRefundView', query: { refund_id: item.refund_id } })">
						<div class="tips-body">
							<span class="tips">时间: {{ $moment.unix(item.add_time).format('YYYY年MM月DD日') }}</span>
							<span class="title tips statusTips">店铺{{ item.seller_state_text }}，平台{{ item.refund_state_text }}</span>
						</div>
						<div class="order-image" v-if="item.goods_list.length > 0"><img v-bind:src="image.goods_img_480" v-for="image in item.goods_list" /></div>
						<div class="price">
							退款金额 : ￥
					...
					<div class="order-list-opratio">
						...
						<div class="btn">
							<button
								v-if="item.seller_state == 1"
								v-on:click="$router.push({ name: 'SellerRefundForm', query: { refund_id: item.refund_id, refund_type, refund_type } })"
							>
								处理
							</button>
							<button v-if="item.seller_state == 2 && item.return_type == 2 && item.goods_state == 2" v-on:click="receive(item.refund_id)">已收货</button>
							<button v-if="item.seller_state == 2 && item.return_type == 2 && item.goods_state == 2 && item.delay_time > 0" v-on:click="unreceive(item.refund_id)">
								未收货
							</button>
							<button v-if="item.invoice_no && item.express_id > 0" v-on:click="$router.push({ name: 'SellerRefundDeliver', query: { refund_id: item.refund_id } })">
								物流跟踪
							</button>
						</div>
					</div>
...
export default {
	...
	methods: {
		...
		getRefundList(ifReplace) {
			...
			getRefundList(this.params, this.refund_type)
				.then(res => {
					Indicator.close();
