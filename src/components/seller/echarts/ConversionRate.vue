<template>
	<div class="statistics_charts">
		<!-- 头部 -->
		<!-- <head-title title="转化率分析" :iconleft="require('../../../assets/image/icon2/icon-white-back.png')" bgcolor='#626771' color="#FFFFFF"></head-title> -->
		...
		<div class="main" >
			...
			图标
			<div class="echart_cont" :style="{width:widthData}">
				<div id="high"  style="width: 280;height: 280px;">
				</div>
			</div> -->
			<!-- 表格 -->
...
import { getConversion } from '../../../api/sellerStatistics';
...
export default {
	...
	data() {
		return {
			...
			searchArr:[
				...
				{ title:"商品分类", value: 'class_id' },
				{ title:"入口模块", value: 'form_id' },
				{ title:"终端", value: 'terminal_id' },
			],
			// 下拉框 数据
			oneDropDown:{
				sel_timeid:[ { name: '一天', id: '1' }, { name: '一周', id: '2' }, { name: '一个月', id: '3' }],
				goods_id:[ { name: '全部', id: '0' } ],
				class_id:[ { name: '全部', id: '0' } ],
				form_id:[ { name: '全部', id: '0' }, { name: '零售', id: '1' }, { name: '团购', id: '2' }, { name: '零售折扣', id: '3' }, { name: '团购折扣', id: '4' }, { name: '零售抢购', id: '5' }, { name: '零售抢购', id: '6' }],
				terminal_id:[ { name: '全部', id: '0' }, { name: 'APP', id: '1' }, { name: '公众号', id: '2' }, { name: 'H5端', id: '3' }, { name: 'PC端', id: '4' }],
			},
	...
	methods: {
		...
		loading() {
			...
			getConversion(this.seller.seller_id,this.searchObj).then(res => {
				if (res) {
					...
					this.oneDropDown.class_id = [{name:"全部",id:0}];
					
					seltag.goods_class.forEach( ( item, index ) => {
						this.oneDropDown.class_id.push(
							{
					...
					if( this.searchObj.sel_timeid == 1 ){
						...
						this.option.series[0].data = [ obj.view ];
						this.option.series[1].data = [ obj.cart ];
						this.option.series[2].data = [ obj.pay ];
						...
						for( var i = 1; i<arr.length; i++ ){
							...
							this.optionZ.series[2].data[i-1] = item[3].val;							
						}
