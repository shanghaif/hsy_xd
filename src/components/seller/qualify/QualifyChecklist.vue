      ...
      <div class="over">
        <div class="li_cont" 
          ...
          <div class="li_top">
            ...
            <div>生成日期：{{item.add_time|fil_dateType}}</div>
          </div>
          <!-- 上部分结束 -->
        </div>
        <div v-if="!this.list.length" style="text-align:center;padding-top:6.8rem">暂无合格证</div>  
      </div>  
    </div>
  </mt-popup>  
</div>
...
import { getQualifyPut } from "../../../api/sellerQualify";//获取 证书列表
export default {
  data(){
    return{
      showState:false,
