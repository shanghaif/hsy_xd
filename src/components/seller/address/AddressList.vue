					...
					<div class="bottom-wrapper">
						<div class="bottom-left-wrapper" @click="onDefault(item)">
							<label class="subtitle">
								<i v-if="item.daddress_isdefault == 1" class="iconfont">&#xe69d;</i>
								<i v-else class="iconfont">&#xe69e;</i>
								默认地址
							</label>
						</div>
						<div class="bottom-right-wrapper">
							<div class="edit-wrapper" @click="onEdit(item.daddress_id)"><label class="subtitle">编辑</label></div>
							<div class="edit-wrapper delete-wrapper" @click="onDelete(item.daddress_id)"><label class="subtitle">删除</label></div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div v-else><empty-record></empty-record></div>
	</div>
</template>
