<template>
	<div class="container">
		<div class="top-wrap">
			...
			<div class="order-header">
				<div class="seller-search">
					...
					<!-- <div class="" @click="getGoodsList()"></div> -->
				</div>
				<ul>
			...
			<div v-if="goodsList.length > 0">
				<div class="goods-item" v-for="(item, index) in goodsList" :key="index">
					...
					<!-- <div class="goods-btn">
						...
						<mt-button size="small" type="danger" class="" @click="drop_goods(item.goods_commonid)">删除</mt-button>
					</div> -->
					<ul class="goods-btn" v-show="index == i">
						...
						<li @click="downBarcode(item.goods_commonid)">溯源码</li>
						<!-- <li @click="drop_goods(item.goods_commonid)">删除</li> -->
					</ul>
				</div>
			</div>
		...
		<div class="add-button" @click="goods_add"><span>新增商品</span></div>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		goods_edit(commonid, class_id) {
			this.$router.push({ name: 'SellerGoodsForm2', query: { commonid: commonid, class_id: class_id, editType:true  } });
		},

		// 新发布
		goods_new(commonid, class_id) {
			this.$router.push({ name: 'SellerGoodsForm2', query: { commonid: commonid, class_id: class_id, editType:false } });
		},
		
		// 下载二维码
		downBarcode(commonid){
			this.$router.push( { name: "SellerBarcode",query:{goods_commonid:commonid} } )
		},
