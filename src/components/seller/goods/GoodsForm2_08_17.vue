...
<div class="container goods-form-2">
    ...
    <div @click="$refs.unitpicker.currentValue = true">
      <mt-field label="单位" v-model="goods_unit_name"></mt-field>
    </div>
    ...
    <mt-field label="团长返点" type="number" @blur.native.capture="changeField($event,'团长返点')"  :readonly="ifSpec" v-model="goods.goods_b_price"></mt-field>
    ...
    <mt-field label="商品库存" :readonly="ifSpec||commonid!=null" v-model="goods.goods_storage"></mt-field>
...
<div class="label">商品安全信息</div>
 ...
 <div  @click="qualifyShow"><mt-cell title="食品合格证" is-link>{{certificate_no?certificate_no:(certificate_no==null?"已选中":"")}}</mt-cell></div>
        ...
        <mt-popup v-model="showSpecValue" position="right" class="common-popup-wrapper">
            ...
            <div class="common-popup-content">
                <div v-for="(spvalue_key,k) in specArray.spvalue_key" :key="k">
                    ...
                    <div @click="setUnitIndex(k)"><mt-field label="单位" v-model="spec[spvalue_key].goods_unit_name"></mt-field></div>
...
//alert(s.indexOf("|"))
  ...
  computed: {
    ...
    qualifyState:function(){
      return (this.info.storeclass_id == "32" || this.info.storeclass_id == "41" || this.info.storeclass_id == "42")
    },
  ...
  methods: {
    ...
    changeField(e,name){
      if( e.target.value>1 || e.target.value<0.01 ){
        switch(name) {
          case "零售折扣":
    ...
    onUnit(values){
      if(this.nowUnit == null){
        this.goods_unit_name = values[0].name;
        this.goods.goods_unit_id = values[0].id;
      }else{
        // this.unitIndex[this.nowUnit] = {
        //   name:values[0].name,
        //   id:values[0].id,
        // }
        this.spec[this.specArray.spvalue_key[this.nowUnit]].goods_unit_name = values[0].name;
        this.spec[this.specArray.spvalue_key[this.nowUnit]].goods_unit_id = values[0].id;
    ...
    nextStep () {
      ...
      console.log( spec );
      return false;
      goodsSave(this.certificate_goods,this.class_id, this.commonid, this.goods, this.bodyList, this.sgcate_id, spec, this.sp_name, sp_val, this.selectAttr, this.g_fccount, this.g_fcprefix).then(res => {
        if (!this.commonid) {
